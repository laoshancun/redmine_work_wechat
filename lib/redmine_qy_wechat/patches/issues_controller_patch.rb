      ...
      def send_by_wechat(send_people_wx)
        ...
        begin
          if @group_client.is_valid?
            ...
            #issue
            #填写确认并应用的应用AgentId
            @group_client.message.send_text(send_people_wx, "", "", app_id,"#{l(:msg_focus)} <a href=\'" + Setting.host_name + "/issues/#{@issue.id}\'>#{@issue.tracker} ##{@issue.id}: #{@issue.subject}</a> #{l(:msg_by)} <a href=\'javascript:void(0);\'>#{@issue.author}</a> #{l(:msg_created)}")
          end
        rescue
        end  
      end
